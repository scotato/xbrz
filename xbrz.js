    ...
    for (let r = 0; r < maxRots; r++) {
      ...
      for (let i = 0; i < maxScale; i++) {
        for (let j = 0; j < maxScale; j++) {
          matrixRotation[nr + i * maxScale + j] = buildMatrixRotation(
            r,
            i,
            j,
            n
...
function preProcessCorners(ker4x4) {
  blendResult.reset();
  if (
    (ker4x4.f === ker4x4.g && ker4x4.j === ker4x4.k) ||
...
function scalePixel(scaler, rotDeg, ker3x3, trg, trgi, trgWidth, blendInfo) {
  ...
  if (BlendInfo.getBottomR(blend) >= BlendType.BLEND_DOMINANT) {
    ...
    BlendInfo.getBottomL(blend) !== BlendType.BLEND_NONE &&
    !eq(e, c)
  ) {
    doLineBlend = false;
  } else {
  ...
  const haveShallowLine =
    config.steepDirectionThreshold * fg <= hc && e !== g && d !== g;
  const haveSteepLine =
    config.steepDirectionThreshold * hc <= fg && e !== c && b !== c;
...
export function scaleImage(
  ...
  if (yFirst > 0) {
    ...
    for (let x = 0; x < srcWidth; ++x) {
      ...
      if (x + 1 < srcWidth) {
        preProcBuffer[x + 1] = BlendInfo.setTopL(
          preProcBuffer[x + 1] & 0xff,
          blendResult.k
        );
      }
    }
  ...
  for (let y = yFirst; y < yLast; ++y) {
    ...
    for (let x = 0; x < srcWidth; ++x, trgi += scaleSize) {
      ...
      const x_p2 = Math.min(x + 2, srcWidth - 1);
      {
        ...
        if (x + 1 < srcWidth) {
          preProcBuffer[x + 1] = BlendInfo.setBottomL(
            preProcBuffer[x + 1],
            blendResult.g
          );
        }
